...
public class SC_RaceProgress : MonoBehaviour
{
    ...
    private SC_PhysicObject physicsObject;
    private SC_Car_npc_predictive m_npc;
    ...
    private void Start()
    {
        ...
        physicsObject = GetComponent<SC_PhysicObject>();
        m_npc = GetComponent<SC_Car_npc_predictive>();
    }
    public void UpdateCurrentCheckpoint(int new_checkpoint)
    {
        if (current_checkpoint == new_checkpoint) return;
        new_checkpoint = new_checkpoint % m_checkpointManager.checkpoints.Length;
        ...
        if (m_npc != null)
            m_npc.SetNextPoint(m_checkpointManager.checkpoints[current_checkpoint].GetRacePoint());
    }

    public void UpdatePressure(float new_pressure, SC_PhysicObject[] near_objects)
    {
        if (m_npc != null)
        {
            m_npc.pressure = new_pressure;
            m_npc.near_cars = near_objects;
        }
    }
    ...
    {
        Transform next = m_checkpointManager.checkpoints[current_checkpoint % m_checkpointManager.checkpoints.Length].transform;
    ...
    public SC_PhysicObject GetPhysicObject() => physicsObject;
}